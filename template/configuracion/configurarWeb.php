<div class="container">
	<form role="form"  Method="POST" action="<?php echo $GLOBALS['CONTROLLER_URL'].'configuracionController.php';?>" >
		<h2>Configurar Web</h2>
		
		<div class="form-group">
			<label for="inputNombre" class="sr-only">Nombre del Concurso</label>
			<input type="text" id="inputNombre" class="form-control" placeholder="Nombre del Concurso" name="nombreConcurso" value="<?php echo $conf->nombre; ?>" required autofocus>
		</div>
		
		<div class="form-group">
			<label for="inputFechaInicio" class="sr-only">Fecha de Inicio</label>
			<input type="text" id="inputFechaInicio" class="form-control" placeholder="Fecha de Inicio" name="fechaInicio" value="<?php echo $conf->f_inicio; ?>" required>			
		</div>
		
		<div class="form-group">
			<label for="inputFechaFin" class="sr-only">Fecha de Fin</label>
			<input type="text" id="inputFechaFin" class="form-control" placeholder="Fecha de Fin" name="fechaFin" value="<?php echo $conf->f_fin; ?>" required>
		</div>
		
		<div class="form-group">
			<label for="inputDescripcion" class="sr-only">Descripción</label>
			<textarea id="inputDescripcion"  class="form-control"  placeholder="Descripcion" name="descripcionConcurso" > <?php echo $conf->descripcion; ?></textarea>			
		</div>
